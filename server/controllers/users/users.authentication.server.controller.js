...
exports.sendOtpToUpdateMobileNumber = async function (req, response) {
	try {
		...
		if (!req.body.phoneNumber) {
			...
			let userExists = await models.users.findOne({ phoneNumber: req.body.phoneNumber, _id: { $nin: req.userInfo._id } });
			if (userExists) {
				return response.status(400).send({
					success: false,
	...
	try {
		...
		if (!validOtp) {
			...
			if (body.location && body.location.latitude && body.location.longitude) {
				...
				userToCreate.locationLongLat.coordinates = [body.location.longitude, body.location.latitude];
			...
			let addNewUser = db.collection("tokens").doc(id).set({
...
exports.signin = async function (req, response) {
	try {
		...
		if (Object.keys(userToUpdate).length > 0) {
			userInfo = await models.users.findOneAndUpdate({ _id: userInfo._id }, userToUpdate, { new: true })
		}
		...
		let getDoc = docRef.get()
			.then(doc => {
				if (!doc.exists) {
					...
					let here = db.collection("tokens").doc(doc.id).update({
						deviceTokens: admin.firestore.FieldValue.arrayUnion(body.deviceToken)
					})
				}
			})
			...
			if (userInfo.error) {
				...
				User.upsertFbUser(token, userInfo, {
					...
					if (err) {
						...
						let fName = input[0],
							lName = input[1]
						let updateName = await models.users.findOneAndUpdate({ 'facebookProvider.id': userInfo.id }, { $set: { firstName: fName, lastName: lName, fullName: userInfo.name } }, { new: true })
						...
						if (Object.keys(jsonToUPdate).length > 0) {
							userHere = await models.users.findOneAndUpdate({ 'facebookProvider.id': userInfo.id }, jsonToUPdate, { new: true })
						}
						...
						res.status(200).send({
							...
							data: {
								...
								hasAnsweredQuestions: userHere.hasAnsweredQuestions || false,
